...
export default function PreferencesModal({ isOpen, onSave, onClose, initialShows = [], userEmail }: PreferencesModalProps) {
    ...
    return (
        ...
        >
            <DialogContent
                className={cn(
                    "max-w-lg w-full bg-card border border-border text-foreground p-0 shadow-2xl overflow-hidden rounded-3xl",
            ...
            >
                ...
                <div className="px-8 pt-10 pb-4 shrink-0">
                    ...
                    <DialogDescription className="text-muted-foreground font-light text-lg mt-2 leading-relaxed">
                ...
                <div className="px-8 space-y-8 overflow-y-auto max-h-[50vh] custom-scrollbar">
                    ...
                    <div className="relative group pt-1">
                        ...
                        <div className="relative flex items-center bg-muted/50 border border-border rounded-full px-5 py-4 shadow-inner transition-colors group-focus-within:border-emerald-500/30 group-focus-within:bg-muted">
                            <Search className="w-5 h-5 text-muted-foreground mr-3 group-focus-within:text-emerald-500 transition-colors" />
                            <input
                                ref={inputRef}
                                type="text"
                                value={inputValue}
                                ...
                                className="w-full bg-transparent border-none focus:outline-none text-foreground placeholder:text-muted-foreground font-light tracking-wide text-lg"
                            />
                            {inputValue && (
                                <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-widest mr-2 animate-in fade-in">
                                    Enter
                                </span>
                            )}
                        </div>
                        ...
                        <div className="min-h-[40px] flex flex-wrap gap-2.5">
                             <AnimatePresence mode="popLayout">
                                {selectedShows.map((show) => (
                                    ...
                                    >
                                        <Badge
                                            className="cursor-pointer bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-100 border border-emerald-500/20 pl-4 pr-3 py-2 rounded-full text-sm font-medium tracking-wide flex items-center gap-2 shadow-[0_0_15px_-3px_rgba(16,185,129,0.1)] hover:shadow-[0_0_20px_-3px_rgba(16,185,129,0.2)] transition-all group"
                                            onClick={() => removeShow(show)}
                                        >
                                            {show}
                                            <X size={14} className="opacity-50 group-hover:opacity-100 text-emerald-600 dark:text-emerald-400" />
                                        </Badge>
                                    </motion.div>
                                ))}
                            </AnimatePresence>
                            {selectedShows.length === 0 && (
                                <div className="w-full text-center py-6 border border-dashed border-border rounded-2xl bg-foreground/5">
                                    <span className="text-muted-foreground italic text-sm font-light">
                    ...
                    <div className="space-y-4 pb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {suggestions.map((show) => (
                                <motion.button
                                    key={show}
                                    whileHover={{ scale: 1.05, backgroundColor: "var(--foreground-5)", borderColor: "var(--border)" }}
                                    whileTap={{ scale: 0.95 }}
                                    onClick={() => addShow(show)}
                                    className="cursor-pointer px-4 py-2 rounded-full text-sm font-light bg-muted/40 border border-border/50 text-muted-foreground transition-all duration-300"
                                >
                                    {show}
                                </motion.button>
                            ))}
                ...
                {/* Footer */}
                <div className="p-8 mt-2 bg-gradient-to-t from-background/50 to-transparent shrink-0">
                    <Button
                        onClick={handleSave}
